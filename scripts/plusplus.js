...
module.exports = function plusplus (robot) {
  ...
  robot.respond(/(top|bottom) (\d+)/i, function (msg) {
    ...
    amount = parseInt(msg.match[2], 10) || 10
    message = []
    tops = scoreKeeper[msg.match[1]](amount)
    if (tops.length > 0) {
      for (i = j = 0, ref = tops.length - 1; ref >= 0 ? j <= ref : j >= ref; i = ref >= 0 ? ++j : --j) {
  ...
  function onMessage (msg) {
    ...
    if (operator === '++') {
      var message = name + ' has ' + score + pluralize(' ' + strings.point, score)
      ...
      message = 'ouch! ' + name + ' loses a ' + strings.point
      if (reason) message += ' for ' + reason
      message += '. ' + name + ' now has ' + score + pluralize(' ' + strings.point, score)
    }
  ...
  function getScore (msg) {
    ...
    if (typeof reasons === 'object' && Object.keys(reasons).length > 0) {
      ...
      message += _.reduce(reasons, function formatScore (memo, val, key) {
        var result = memo += '\n' + key + ': ' + val + pluralize(' ' + strings.point, val)
        return result
      }, '')
    }
